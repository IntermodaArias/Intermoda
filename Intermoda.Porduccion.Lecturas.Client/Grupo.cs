    ...
    public class Grupo : ObservableObject
    {
        ...
        public static async Task<Grupo> Update(Grupo reg)
        {
            try
            {
                using (_client = new DataServiceClient())
                {
                    ...
                    var model = await _client.GrupoUpdateAsync(business);
        ...
        public static async Task Delete(int grupoId)
        {
            try
            {
                using (_client = new DataServiceClient())
                {
                    await _client.GrupoDeleteAsync(grupoId);
                }
            }
            catch (Exception exception)
            {
        ...
        public static async Task<Grupo> Get(int grupoId)
        {
            try
            {
                using (_client = new DataServiceClient())
                {
                    var model = await _client.GrupoGetAsync(grupoId);
                    return new Grupo
                    {
                        Id = model.Id,
                        FechaInicio = model.FechaInicio,
        ...
        public static async Task<List<Grupo>> GetAll()
        {
            try
            {
                using (_client = new DataServiceClient())
                {
                    var lista = await _client.GrupoGetAllAsync();
        ...
        public static async Task<List<Grupo>> GetAllActive() {
            try
            {
                using (_client = new DataServiceClient())
                {
                    var lista = await _client.GrupoGetAllActivosAsync();
                    return lista.Select(model => new Grupo
                    {
                        Id = model.Id,
                        FechaInicio = model.FechaInicio,
        ...
        public static async Task<Grupo> GetByLinea(int lineaId)
        {
            try
            {
                using (_client = new DataServiceClient())
                {
                    var model = await _client.GrupoGetByLineaAsync(lineaId);
                    return new Grupo
                    {
                        Id = model.Id,
                        FechaInicio = model.FechaInicio,
        ...
        public static async Task CopiarSemana(DateTime desde, DateTime hasta)
        {
            try
            {
                using (_client = new DataServiceClient())
                {
                    await _client.GrupoCopiarSemanaAsync(desde, hasta);
                }
            }
            catch (Exception exception)
            {
        ...
        public static async Task<bool> HayDataSemana(DateTime fecha)
        {
            try
            {
                using (_client = new DataServiceClient())
                {
                    return await _client.GrupoHayDataSemanaAsync(fecha);
