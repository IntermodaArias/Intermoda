...
namespace Intermoda.Client.Lectura
{
    ...
    {
        private static LineaClient _client;
        ...
        public static async Task Delete(int lineaId) {
            try
            {
                using (_client = new LineaClient())
                {
                    await _client.DeleteAsync(lineaId);
                }
            }
            catch (Exception exception)
            {
        ...
        public static async Task<List<Linea>> GetByGrupo(int grupoId)
        {
            try
            {
                using (_client = new LineaClient())
                {
                    var lista = await _client.GetByGrupoAsync(grupoId);
        ...
        public static async Task<List<Linea>> GetByGrupoActivas(int grupoId)
        {
            try
            {
                using (_client = new LineaClient())
                {
                    var lista = await _client.GetByGrupoActivasAsync(grupoId);
                    return lista.Select(model => new Linea
                    {
                        Id = model.Id,
                        GrupoId = model.GrupoId,
