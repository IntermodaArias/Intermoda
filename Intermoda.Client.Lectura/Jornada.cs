...
using Intermoda.ClientProxy.Lectura.JornadaServiceReference;

namespace Intermoda.Client.Lectura
{
    public class Jornada : ObservableObject
    {
        private static JornadaClient _client;
        ...
        public static async Task<Jornada> Update(Jornada reg)
        {
            try
            {
                using (_client = new JornadaClient())
                {
        ...
        public static async Task Delete(int jornadaId)
        {
            try
            {
                using (_client = new JornadaClient())
                {
                    await _client.DeleteAsync(jornadaId);
                }
            }
            catch (Exception exception)
            {
        ...
        public static async Task<Jornada> Get(int jornadaId)
        {
            try
            {
                using (_client = new JornadaClient())
                {
                    var model = await _client.GetAsync(jornadaId);
